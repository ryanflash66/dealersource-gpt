...
12. **Dashboard template integration:** The published `tokens.css` and `dashboard.css` are shipped verbatim except for the required GPT accent token, `#10A37F`. Runtime data is rendered into the template's four page layouts and component class names. App-only behavior and accessibility rules live in `styles.css`.
