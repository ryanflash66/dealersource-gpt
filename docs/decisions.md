...
18. **Dashboard support data:** `report.json` includes additive listing summaries (`listing_id`, `source_id`, URL, and stated rent) so the drawer can identify merged records when the dashboard is rebuilt from contract outputs alone. Required contract keys and pipeline decisions are unchanged.
19. **Dashboard case dates:** An open case uses its outbound message time as the opened time and adds `mail.followup_days` for the next action. If a report predates message detail, the logical run date is the fallback.
20. **Staleness and expiry windows:** A dashboard report becomes stale after 24 hours. The Exceptions view lists evidence that is expired or expires within seven days, which keeps the list actionable without changing gate evaluation.
21. **Healthy exception states:** When sending is enabled or no source failed, the Exceptions view states that explicitly. Sources marked `terms_status: prohibited` in `sources.yaml` remain listed with their recorded reasons even when the run has no failures.
