...
export class Gmail {
 ...
 async replies(store:Store):Promise<Row[]>{
  ...
  // Include relevant replies/DSNs that Gmail placed in a new thread, not the whole inbox.
  let token='';do{const url=new URL('https://gmail.googleapis.com/gmail/v1/users/me/messages');url.searchParams.set('q','newer_than:2d "DealerSource verification"');url.searchParams.set('maxResults','100');if(token)url.searchParams.set('pageToken',token);const page=await this.registry.transport.request(url.toString(),{headers:{Authorization:'Bearer '+this.token}},{provider:'email:gmail'});for(const item of page.messages??[])if(item.threadId)threads.add(item.threadId);token=page.nextPageToken??'';}while(token);
  for(const thread of threads){const result=await this.registry.transport.request('https://gmail.googleapis.com/gmail/v1/users/me/threads/'+encodeURIComponent(thread)+'?format=full',{headers:{Authorization:'Bearer '+this.token}},{provider:'email:gmail'});
   let outgoing=store.all('messages').find(m=>m.thread_id===thread&&m.direction==='outbound');if(!outgoing){const decoded:string[]=[];const inspect=(p:Row)=>{if(p.body?.data)decoded.push(Buffer.from(p.body.data,'base64url').toString('utf8').slice(0,50000));for(const part of p.parts??[])inspect(part);};for(const m of result.messages??[])inspect(m.payload??{});const wire=JSON.stringify(result)+decoded.join('\n');outgoing=store.all('messages').find(m=>m.direction==='outbound'&&wire.includes(m.id+'@dealersource.local'));if(!outgoing){const c=store.all('cases').find(c=>wire.includes(c.id));if(c)outgoing=store.all('messages').find(m=>m.case_id===c.id&&m.direction==='outbound');}}if(!outgoing)continue;
   for(const message of result.messages??[]){if(message.id===outgoing.provider_id||store.all('messages').some(m=>m.provider_id===message.id))continue;const headers=message.payload?.headers??[];const get=(n:string)=>headers.find((h:Row)=>h.name.toLowerCase()===n)?.value??'';const from=(get('from').match(/<([^>]+)>/)?.[1]??get('from')).toLowerCase();
    const pieces:string[]=[];let deliveryStatus=false;const walk=(part:Row)=>{if(part.mimeType==='message/delivery-status')deliveryStatus=true;if(['text/plain','message/delivery-status'].includes(part.mimeType)&&part.body?.data)pieces.push(Buffer.from(part.body.data,'base64url').toString('utf8'));for(const p of part.parts??[])walk(p)};walk(message.payload??{});
    const body=pieces.join('\n').slice(0,50000);out.push({external_id:message.id,case_id:outgoing.case_id,from,body,fact:store.all('cases').find(c=>c.id===outgoing.case_id)?.fact,autoreply:!!get('auto-submitted'),bounce:deliveryStatus&&/Action:\s*failed|Status:\s*5\./i.test(body),outbound_id:outgoing.id,synthetic:false});
   }
  }
  return out;
 }
